...
if(typeof Muse == "undefined") window.Muse = {}; window.Muse.assets = {"required":["museutils.js", "museconfig.js", "jquery.watch.js", "require.js", "consumer-orders.css"], "outOfDate":[]};
  ...
  <title>Producer Orders</title>
  <!-- CSS -->
  <link rel="stylesheet" type="text/css" href="css/site_global.css?crc=444006867"/>
  <link rel="stylesheet" type="text/css" href="css/master_a-master.css?crc=4276979596"/>
  <link rel="stylesheet" type="text/css" href="css/consumer-orders.css?crc=244484862" id="pagesheet"/>
    <!--HTML Widget code-->
...
<div class='mf-menu'>
  ...
  <div class='menuContent'>
   ...
   <div class="colelem" id="u427"><!-- content --></div>
   <div class="verticalspacer" data-offset-top="224" data-content-above-spacer="224" data-content-below-spacer="276"><div class="w3-container">
  <h2>View all orders</h2>
  <a class="btn btn-small btn-info" href="neworderfile.php">
  <i class="icon-plus"></i> Add Order</a>
  <table class="w3-table-all w3-hoverable">
    ...
    $sql = "SELECT * FROM orders WHERE type = 'tribe'";
    $result = $conn->query($sql);
    
    if ($result->num_rows > 0) {
        // output data of each row
        while($row = $result->fetch_assoc()) {
            $form = '<form action="order-info.php"><input type="submit" name="number" value="'. $row["number"].'"></form> ';
            echo "<tr><td>" . $row["date"]. '</td><td>'.$row["number"].'</td><td> ' . $row["account"]. "</td><td>" . $row["address"]. "</td><td>" . $row["total"] ."</td><td>" . $form. "</td></tr>";
        }
    } else {
        echo "0 results";
    }
